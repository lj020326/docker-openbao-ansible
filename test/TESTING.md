...
$ 
$ run_docker_compose up -d
$ run_docker_compose exec -T openbao-test fetch_openbao_info.sh --content
$ run_docker_compose exec -T openbao-test fetch_openbao_info.sh --root-token
$ run_docker_compose exec -T openbao-test fetch_openbao_info.sh --admin-token
...
$ run_docker_compose exec -T openbao-test sh -c "fetch_openbao_info.sh --content"
$ run_docker_compose exec -T openbao-test sh -c "ls /vault/config/init.json.enc"
$ run_docker_compose exec -T openbao-test sh -c 'fetch_openbao_info.sh --root-token'
$ run_docker_compose exec openbao-test sh
$ local root_token=$(docker compose --project-name openbao-ansible-test -f test/docker-compose.test.yml --env-file .test/.env.test exec -T "${CONTAINER_SERVICE_NAME}" fetch_openbao_info.sh --root-token)
$ local admin_token=$(docker compose --project-name openbao-ansible-test -f test/docker-compose.test.yml --env-file .test/.env.test exec -T "${CONTAINER_SERVICE_NAME}" fetch_openbao_info.sh --admin-token)
$ 
$ run_docker_compose down
$ rm -fr .test
```
...
Step 11/19 : COPY env_secrets_expand.sh /usr/local/bin/env_secrets_expand.sh
 ---> Using cache
 ---> 25c75796070c
Step 12/19 : COPY fetch_openbao_info.sh /usr/local/bin/fetch_openbao_info.sh
 ---> Using cache
 ---> fbd7950447e4
Step 13/19 : COPY openbao_setup.sh /usr/local/bin/openbao_setup.sh
 ---> Using cache
 ---> 97e047c39208
Step 14/19 : RUN chmod 755 /usr/local/bin/docker-entrypoint.sh   /usr/local/bin/env_secrets_expand.sh   /usr/local/bin/fetch_openbao_info.sh   /usr/local/bin/openbao_setup.sh
 ---> Using cache
 ---> f090b158412f
...
Cluster ID      3fd7a2ed-5fdc-4880-b14c-e706a37b6411
HA Enabled      false
INFO: Validating fetch_openbao_info.sh content output...
