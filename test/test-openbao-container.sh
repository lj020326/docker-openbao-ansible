...
test_initialization() {
    ...
    log_info "Validating fetch_openbao_info.sh content output..."
    local DECRYPTED_CONTENT=$(run_docker_compose exec -T "${OPENBAO_TEST_SERVICE_NAME}" sh -c "fetch_openbao_info.sh --content")
...
test_setup_validation() {
    ...
    ADMIN_TOKEN=$(run_docker_compose exec -T "${OPENBAO_TEST_SERVICE_NAME}" fetch_openbao_info.sh --admin-token)
    ...
    local ROOT_TOKEN=$(run_docker_compose exec -T "${OPENBAO_TEST_SERVICE_NAME}" sh -c "fetch_openbao_info.sh --root-token")
...
test_data_integrity() {
    ...
    if [ -z "${ROOT_TOKEN}" ]; then
        log_error "Failed to retrieve root token via fetch_openbao_info.sh."
        return 1
    fi
    ...
    local ADMIN_TOKEN=$(run_docker_compose exec -T "${OPENBAO_TEST_SERVICE_NAME}" sh -c "fetch_openbao_info.sh --admin-token")
    if [ -z "$ADMIN_TOKEN" ] || [ "$ADMIN_TOKEN" = "null" ]; then
    ...
    if ! run_docker_compose exec -T "${OPENBAO_TEST_SERVICE_NAME}" bao kv put kv/test-secret mykey=myvalue > /dev/null; then
...
test_external_connectivity() {
    ...
    local ROOT_TOKEN=$(run_docker_compose exec -T "${OPENBAO_TEST_SERVICE_NAME}" fetch_openbao_info.sh --root-token)
